...
class MainActivity : FlutterActivity() {
        ...
        cameraProviderFuture.addListener({
            try {
                ...
                imageAnalysis.setAnalyzer(executor) { imageProxy ->
                    ...
                    // Unbind and send result on the main thread
                    runOnUiThread {
                        try {
                            provider.unbindAll()
                        } catch (_: Exception) {}
    ...
    private fun processFrame(image: ImageProxy): ByteArray {
        ...
        val yBuffer = yPlane.buffer.duplicate()  // duplicate to avoid position side-effects
        val width = image.width
        val height = image.height
        val rowStride = yPlane.rowStride
        val pixelStride = yPlane.pixelStride

        // ── 1. Copy Y-plane to a contiguous byte array ──
        ...
        if (rowStride == width && pixelStride == 1) {
            ...
            // Slow path: row-by-row copy accounting for stride
            val rowBuffer = ByteArray(rowStride)
            for (row in 0 until height) {
                val remaining = yBuffer.remaining()
                val toRead = if (row == height - 1) {
                    // Last row may not have full stride padding
                ...
                if (row == height - 1 && remaining < rowStride) {
                    // Last row: read only what's available
        ...
        // ── 2. Find brightest region (ROI) using 8×8 block scanning ──
        val blockSize = 8
        val roiBlocks = 4  // 4 blocks = 32px ROI
        var bestSum = 0L
        var bestX = 0
        var bestY = 0
        ...
        // ── 3. Draw bounding box (white, 2px thick) ──
        drawRect(yBytes, width, height, roiX, roiY, roiSize, roiSize, 2)

        // ── 4. Convert Y-plane to NV21 (fill UV with 128 for grayscale) and encode JPEG ──
        val nv21 = ByteArray(width * height * 3 / 2)
        System.arraycopy(yBytes, 0, nv21, 0, width * height)
        // Fill UV plane with 128 (neutral gray)
